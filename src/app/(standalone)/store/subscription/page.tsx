...
const VendorPlansPage = () => {
  ...
  const handleSelectPlan = (planId: string) => { selectPlan({ planId }, { onSuccess: (res) => {dispatch(setSignin(res.data));router.push(ROUTES.STORE.SETUP);},});};

  if (isLoading) return (<div className="flex h-screen items-center justify-center bg-gray-50"><Spin size="large" /></div>);
              ...
              <div className="p-8 flex flex-col h-full bg-white">
                ...
                <div className="flex-1">
                  ...
                  <ul className="space-y-4 mb-10">
                    <li className="flex items-center gap-3 text-gray-600"><div className="flex-shrink-0 w-6 h-6 rounded-full bg-brand-50 flex items-center justify-center"><RiCheckLine className="text-brand-600" /></div><span className="font-medium">{plan.productLimit} Products</span></li>
                    <li className="flex items-center gap-3 text-gray-600"><div className="flex-shrink-0 w-6 h-6 rounded-full bg-brand-50 flex items-center justify-center"><RiCheckLine className="text-brand-600" /></div><span className="font-medium">{plan.themeLimit} Themes</span></li>
                    <li className="flex items-center gap-3 text-gray-600"><div className="flex-shrink-0 w-6 h-6 rounded-full bg-brand-50 flex items-center justify-center"><RiCheckLine className="text-brand-600" /></div><span className="font-medium">{plan.orderLimit} Orders / Month</span></li>
                    <li className="flex items-center gap-3 text-gray-600"><div className="flex-shrink-0 w-6 h-6 rounded-full bg-brand-50 flex items-center justify-center"><RiCheckLine className="text-brand-600" /></div><span className="font-medium">{plan.blogLimit} Blog Posts</span></li>
                  </ul>
                </div>
