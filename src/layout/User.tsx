...
const User = () => {
  ...
  return (
    <div className="relative group">
      <CommonProfileAvatar fullName={"user"} profileImage={""} className="max-xsm:text-sm h-10 w-10 max-xsm:h-9 max-xsm:w-9" />
      <div className="fixed lg:absolute max-lg:left-1 max-lg:right-1 lg:right-0 mt-3 flex min-w-[250px] max-w-[330px] flex-col rounded-xl border border-gray-50 bg-white shadow-tooltip dark:border-gray-800 dark:bg-gray-dark z-50 opacity-0 invisible scale-95 translate-y-2 transition-all duration-200 ease-out group-hover:opacity-100 group-hover:visible group-hover:scale-100 group-hover:translate-y-0 ">
        <div className="p-3">
          <div className="flex items-center gap-3">
            <CommonProfileAvatar fullName={"user"} profileImage={""} className="h-9 w-9" />
            <div>
              <span className="block font-semibold text-gray-700 text-theme-lg dark:text-gray-400"> {"user"} </span>
              <span className="block text-theme-xs text-gray-500 dark:text-gray-400">{""} </span>
            </div>
          </div>
        </div>
        ...
        <Link href={ROUTES.AUTH.LOGIN} onClick={() => dispatch(setSignOut())} className="flex items-center gap-3 px-3 py-2 m-2 mx-1 font-medium text-gray-700 rounded-lg group text-theme-sm hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
