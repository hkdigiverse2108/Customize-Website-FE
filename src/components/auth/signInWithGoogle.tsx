...
const SignInWithGoogle = ({ type }: { type: "signup" | "signin" }) => {
  ...
  return (
    <button type="button" onClick={() => handleGoogleLogin()} disabled={isSignupGoogleLoading} className="inline-flex items-center justify-center gap-3 py-3 text-sm font-normal text-gray-700 transition-colors bg-gray-100 rounded-lg px-7 hover:bg-gray-200 hover:text-gray-800 disabled:opacity-50 disabled:cursor-not-allowed w-full">
      ...
      {isSignupGoogleLoading ? "Signing in..." : "Sign in with Google"}
    </button>
  );
};
