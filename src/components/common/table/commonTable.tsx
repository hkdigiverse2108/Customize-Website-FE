...
export const CommonTable = <T extends object>({ loading = false, dataSource, columns = [], pagination = { current: 1, pageSize: 10 }, rowKey = "_id", bordered = false, size = "middle", scroll, onActive, onSearch, ...rest }: CommonTableProps<T>) => {
            ...
            {onActive && (
              <Col xs={24} md={4} xl={3} xxl={2}>
                <div className="flex items-center gap-3 bg-gray-100 px-3 py-3 rounded-lg">
                  ...
                  <Switch size="small" checked={onActive?.value} onChange={onActive?.onChange} />
                </div>
              </Col>
            )}
          </Row>
